...
public class SimulationTest {
    ...
    private Simulation simulation = new Simulation(randomGenerator, boundary, INITIAL_PARTICLE);
    ...
    public void should_stop_when_final_position_is_reached() {
        assertThat(simulation.simulate(new Particle(1, 0)).run()).isEqualTo(new Particle(1, 0));
    }

    @Test
    public void should_move_particle_one_NORTH() {
        when(randomGenerator.nextMove()).thenReturn(new int[]{0, 1});
        assertThat(simulation.simulate(new Particle(0, -2)).run()).isEqualTo(new Particle(0, -1));
    }
    ...
    public void should_move_particle_one_EAST() {
        ...
        assertThat(simulation.simulate(new Particle(-2, 0)).run()).isEqualTo(new Particle(-1, 0));
    }
    ...
    public void should_move_particle_2_NORTH() {
        when(randomGenerator.nextMove()).thenReturn(new int[]{0, 1});
        assertThat(simulation.simulate(new Particle(0, -3)).run()).isEqualTo(new Particle(0, -1));
    ...
    public void should_rebound_on_EAST_boundary() {
        ...
        when(randomGenerator.nextMove()).thenReturn(new int[]{-1, 0});
        assertThat(simulation.simulate(new Particle(4, 0)).run()).isEqualTo(new Particle(1, 0));
    }
    ...
    public void should_rebound_on_NORTH_boundary() {
        ...
        when(randomGenerator.nextMove()).thenReturn(new int[]{0, -1});
        assertThat(simulation.simulate(new Particle(0, 4)).run()).isEqualTo(new Particle(0, 1));
    }
