...
public class SimpleCalculatorTest {
    ...
    public void should_return_0_on_empty_string() {
        when(parser.parseToOperation("")).thenReturn(new Operation(new int[]{0}, new Addition()));
    ...
    public void should_return_element_of_a_singleton() {
        when(parser.parseToOperation("8")).thenReturn(new Operation(new int[]{8}, new Addition()));
        assertThat(calculator.add("8")).isEqualTo(8);
    }

    @Test
    public void should_add_two_numbers() {
        when(parser.parseToOperation("8,10")).thenReturn(new Operation(new int[]{8, 10}, new Addition()));
